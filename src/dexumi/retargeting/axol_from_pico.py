"""Retarget PICO upper-body poses to the Axol IK solver."""
...
def make_rest_q(solver: KinematicsSolver) -> np.ndarray:
    """Create the full Axol joint vector for the visual rest pose."""
...
def robot_link_pose(
    ...
    """Return ``(position, rotation_matrix)`` for one Axol link."""
...
def axol_link_positions(
    solver: KinematicsSolver,
    q: np.ndarray,
    link_indices: list[int],
) -> np.ndarray:
    """Return link positions for drawing the Axol skeleton."""
...
class PicoToAxolArmRetargeter:
    """Relative-position retargeter from PICO upper body to Axol arms."""
    ...
    def retarget_frame(self, body_pose: np.ndarray, q_current: np.ndarray) -> np.ndarray:
        """Solve one frame of PICO upper-body data into an Axol joint vector."""
    ...
    def split_for_sim(self, q: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        """Convert the full IK vector into left/right Axol command arrays."""
...
def move_retargeter_to_front_workspace(
    ...
    *,
    ...
    """Use a chest/front Axol workspace instead of the raw URDF rest pose."""
